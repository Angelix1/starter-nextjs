...
async function sendLink() {
  ...
  let res = await fetch(`${host}/api?getVidData=${encodeURIComponent(F)}`).catch(() => false);
  if(res) {
...
function loadTrack(track_index) {
  ...
  fetch(`${host}/getVidAudio/${track_list[track_index].videoId}`)
    .then(a => a.json())
    .then(data => track_list[track_index].url = data.url);
  

  // Load a new track

  if(!track_list[track_index].url) return nextTrack;
